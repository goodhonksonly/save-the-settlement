...
export function Door({ unlockedCount, isOpen = false }: DoorProps) {
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      const timer = setTimeout(() => setAnimatedOpen(true), 800)
    ...
    <div className="flex items-center justify-center min-h-[320px] lg:min-h-[540px]">
      ...
      >
        ...
        <div
          ...
          style={{
            inset: "-18px",
            background: "linear-gradient(180deg, #555 0%, #444 20%, #333 80%, #282828 100%)",
            boxShadow: "0 24px 70px rgba(0,0,0,0.7), inset 0 1px 0 rgba(255,255,255,0.07), inset 2px 0 6px rgba(0,0,0,0.3), inset -2px 0 6px rgba(0,0,0,0.3)",
          }}
        />
        {/* Inner frame reveal — slightly lighter to show depth */}
        <div
          className="absolute rounded-t-sm"
          style={{
            inset: "-7px",
        ...
        <div
          ...
          style={{ background: "radial-gradient(ellipse at 50% 40%, #0a0c0e, #020304)" }}
        />
        ...
        <div
          ...
          style={{
            ...
            borderRadius: "2px",
            boxShadow: "0 4px 14px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.15)",
          }}
        />
      </div>
    </div>
...
function DoorLeaf({ side }: { side: "left" | "right" }) {
  ...
  return (
    <div
      ...
      style={{
        background: "linear-gradient(180deg, #d8c8a4 0%, #caba8e 20%, #d2c296 50%, #c6b68a 80%, #cec094 100%)",
        boxShadow: isLeft
    ...
    >
      ...
      {/* Recessed door panels — top and bottom */}
      {[12, 55].map((top) => (
        <div
          key={top}
          className="absolute"
          style={{
            ...
            boxShadow: "inset 2px 2px 5px rgba(0,0,0,0.18), inset -1px -1px 3px rgba(255,255,255,0.12)",
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            boxShadow: "0 2px 6px rgba(0,0,0,0.5), inset 0 1px 2px rgba(255,255,220,0.4)",
          }}
        />
      </div>
    </div>
...
function Hinge({ top, side }: { top: number; side: "left" | "right" }) {
  ...
  return (
    ...
    >
      {/* Hinge screw dots */}
      {[6, 18].map((t) => (
        <div key={t} className="absolute left-1/2 -translate-x-1/2 w-[4px] h-[4px] rounded-full" style={{ top: t, background: "#222", boxShadow: "inset 0 1px 1px rgba(0,0,0,0.8)" }} />
      ))}
    </div>
  )
}
    ...
    <div className="absolute left-0 right-0" style={{ top: `${topPercent}%`, transform: "translateY(-50%)" }}>
      <div
        ...
        style={{
          ...
          boxShadow: "0 3px 7px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.1)",
      ...
      >
        <div className="absolute left-1/2 -translate-x-1/2 -top-[10px] w-[18px] h-[12px]" style={{ borderRadius: "9px 9px 0 0", border: "4px solid #999", borderBottom: "none" }} />
